...
| Process identity, missing/stale PID recovery, unrelated-process protection, single-instance behavior, repeated start, exact-process startup handoff, late-arrival race, discovery failure, and LaunchAgent isolation | `bash Tests/Shell/verify-control-commands.sh` | Passed; handoff stopped initial and late exact processes, preserved a same-basename different-path process, failed closed on process discovery error, and never used `open` |
...
| Local installation | Release build copied to `/Applications/Codex Pet Usage.app`; installed signature, plist, executable architecture, launchd job ownership, and exact-process count checked | Passed; launchd job PID 49271 remained running and all 50 samples over 8.619 seconds observed exactly one installed process |
...
| Badge tracks pet and moves card left at the display edge | Partial | Live-frame origin correction and right-edge card placement pass deterministic tests. Current metadata-only evidence matched JSON overlay `(0, 0, 356, 320)` to the live `com.openai.codex` frame `(0, 30, 356, 320)`, for delta `(0, 30)` and corrected pet rectangle `(0, 96, 80, 87)`; human visual confirmation remains pending |
| 5-hour/7-day values, countdowns, and source render correctly | Partial | The operational log contains 527 structured usage updates; the latest was `source=live, 5h=--, 7d=66`. The log scan found no Authorization, bearer-token, access-token-value, request-body, response-body, or cookie fragments; exact Chinese strings pass presentation tests; visual confirmation pending |
...
| User-scoped Codex-triggered startup | Real `gui/501` registration points to the exact `/Applications` executable and one `~/.codex/.codex-global-state.json` `WatchPaths` trigger; `RunAtLoad`/`KeepAlive` are absent. With Codex active, launchd owned PID 49271 after exact-process handoff; 50 consecutive samples over 8.619 seconds found the job running and exactly one installed process. The isolated verifier covers initial and late exact-process termination, same-basename decoy preservation, fail-closed discovery, inactive `WatchPaths` waiting, and exact-label kickstart without `open` |
